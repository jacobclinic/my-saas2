...
import { ZoomRecordingResponse, ZoomTokenResponse, ZoomUserRecordingsResponse } from './types/zoom.types';
...
export async function deleteRecordingFile(meetingId: string, recordingId: string): Promise<void> {
  const accessToken = await getZoomAccessToken();
  const url = `https://api.zoom.us/v2/meetings/${meetingId}/recordings/${recordingId}`;
  logger.info(`[Zoom] Deleting recording file ${recordingId} for meeting ${meetingId}`);
  ...
  if (!response.ok) {
    ...
    logger.error(`[Zoom] Failed to delete recording file ${recordingId} for meeting ${meetingId}: ${errorText}`);
    throw new Error(`Failed to delete recording file: ${response.statusText}`);
  }

  logger.info(`[Zoom] Successfully deleted recording file ${recordingId} for meeting ${meetingId}`);
}


// Download a recording file as a stream
export async function downloadRecording(downloadUrl: string, fileName: string): Promise<NodeJS.ReadableStream> {
  const accessToken = await getZoomAccessToken();
  ...
  if (!response.ok) {
    logger.error(`[Zoom] Failed to download recording: ${response.status} - ${response.statusText}`);
...
export async function getAllRecordings(from: string, to: string, pageSize = 30): Promise<ZoomUserRecordingsResponse["meetings"]> {
  const accessToken = await getZoomAccessToken();
...
export async function syncZoomRecordings() {
  try {
    ...
    const now = new Date();
    const twentyFourHoursAgo = new Date(now.getTime() - 30*24 * 60 * 60 * 1000);
    ...
    for (const meeting of meetings) {
      ...
      if (fetchError || !session) {
        logger.warn(`[Cron] No session found for Zoom Meeting ID ${zoomMeetingId}, skipping.`);
        continue;
      }
      ...
      if (recordingFiles.length === 0) {
        logger.info(`[Cron] No recording files found for Zoom Meeting ID ${zoomMeetingId}, skipping.`);
        continue;
      }
      
      for (const file of recordingFiles) {
        if (file.status !== "completed") {
          logger.info(`[Cron] Recording file ${file.id} for Zoom Meeting ID ${zoomMeetingId} is not completed, skipping.`);
          continue;
        }

        try {
          // Step 6: Download the recording file
          const fileExtension = file.file_type.toLowerCase() === "mp4" ? "mp4" : file.file_type.toLowerCase() === "m4a" ? "m4a" : "unknown";
          const fileName = `${zoomMeetingId}-${file.id}.${fileExtension}`;
          const fileStream = await downloadRecording(file.download_url, fileName);
          ...
          logger.info(`[Cron] Uploaded recording file ${file.id} to S3 for Zoom Meeting ID ${zoomMeetingId}: ${signedUrl}`);
          
         
          // Step 8: Update the session with the S3 URL
          await updateRecordingUrl(supabase, zoomMeetingId, signedUrl);
          logger.info(`[Cron] Updated recording URL for Zoom Meeting ID ${zoomMeetingId} with file ${file.id}`);
          ...
          logger.error(`[Cron] Error processing recording file ${file.id} for Zoom Meeting ID ${zoomMeetingId}:`, error);
        }
      }
    ...
    logger.info(`[Cron] Zoom recording sync completed. Processed ${processedMeetings} meetings.`);
    return { message: "Zoom recording sync completed", processed: processedMeetings };
  } catch (error) {
