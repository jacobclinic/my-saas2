...
const StudentRegistrationForm = ({ classData }: StudentRegistrationFormrops) => {
    ...
    if (validateForm()) {
			if (validateForm()) {
				const result = await registerStudentAction({
					...
					nameOfClass: '2025 A/L Accounting - Batch 04'
				});
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <Card className="w-full max-w-2xl">
        <CardHeader className="text-center">
          <CardTitle className="text-2xl text-blue-600">Join Your Class</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Class Info */}
            <div className="bg-blue-50 p-4 rounded-lg">
              <h2 className="font-semibold text-lg mb-2">{classData.className}</h2>
