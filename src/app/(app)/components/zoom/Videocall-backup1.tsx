...
const Videocall = (props: { slug: string; JWT: string }) => {
  ...
  useEffect(() => {
    const checkRecordingStatus = async () => {
        if (!client.current || !inSession) return; // Check if client is initialized and in session
        try {
            const recordingClient = await client.current.getRecordingClient();
            const status = await recordingClient.getCloudRecordingStatus();
    ...
    const intervalId = setInterval(checkRecordingStatus, 5000); // Check every 5 seconds

    return () => clearInterval(intervalId); // Clean up on unmount
}, [inSession]); // Add inSession as a dependency
  ...
  const toggleRecording = async () => {
    try {
      if (isRecording) {
        await client.current.stopRecording();
      ...
      {!inSession ? (
        ...
        <div className="flex w-full flex-col justify-around self-center">
          <div className="mt-4 flex w-[30rem] flex-1 justify-around self-center rounded-md bg-white p-4">
            ...
            />
            <Button onClick={toggleRecording} disabled={!inSession}>
